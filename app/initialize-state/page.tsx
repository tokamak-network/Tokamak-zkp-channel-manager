...
export default function InitializeStatePage() {
  const {
    ...
    hasAccess,
    isMounted,
    leaderChannel: hookLeaderChannel,
  } = useLeaderAccess();
  ...
  const getFirstToken = () => {
    const allowedTokens = hookLeaderChannel
    ...
    for (
      ...
      try {
        const keysResponse = await fetch(
          `/api/get-l2-mpt-keys-list?channelId=${hookLeaderChannel.id}&token=${token}`,
          {
        ...
        try {
          ...
          if (keysResult?.keyMap) {
            ...
            try {
              const keyResponse = await fetch(
                `/api/get-l2-mpt-key?participant=${participant}&token=${token}&channelId=${hookLeaderChannel.id}`,
                {
          ...
          try {
            const depositResponse = await fetch(
              `/api/get-participant-deposit?participant=${participant}&token=${token}&channelId=${hookLeaderChannel.id}`,
              {
                signal: AbortSignal.timeout(5000),
              }
            );
        ...
        <main className="px-4 py-8 lg:px-8">
          ...
          {!isConnected ? (
            ...
            <div className="bg-gradient-to-b from-[#1a2347] to-[#0a1930] border border-[#4fc3f7] p-8 text-center shadow-lg shadow-[#4fc3f7]/20">
              ...
              <p className="text-gray-300 mb-4">
                You don't have a channel in "Initialized" state that can be
                opened
              </p>
              <p className="text-sm text-gray-400">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                  ...
                  <div className="text-center p-3 sm:p-4 bg-[#0a1930]/50 border border-[#4fc3f7]/30">
                    ...
                    <div className="text-lg sm:text-xl font-bold text-[#4fc3f7]">
                      {getChannelStateName(hookLeaderChannel.stats[2])}
                    </div>
                  </div>
                  <div className="text-center p-3 sm:p-4 bg-[#0a1930]/50 border border-[#4fc3f7]/30">
                    <div className="text-xs sm:text-sm text-gray-400">
                ...
                <div className="text-center">
                  ...
                  <div className="mt-4 p-3 bg-[#4fc3f7]/10 border border-[#4fc3f7]/50 text-center">
                    ...
                    <div className="text-xs text-gray-400 mt-2">
                      ...
                      × {(hookLeaderChannel?.stats?.[1] as any[])?.length || 0}{" "}
                      ={" "}
                      {(channelParticipants?.length || 0) *
                        ((hookLeaderChannel?.stats?.[1] as any[])?.length ||
                          0)}{" "}
                      entries
                    </div>
                  </div>
