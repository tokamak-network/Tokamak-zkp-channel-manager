...
async function searchLogsInChunks(
  ...
  while (currentBlock <= endBlock && iterations < maxIterations) {
    ...
    if (iterations % 1000 === 0) {
      console.log(
        `  Searched ${iterations} chunks, current block: ${currentBlock}`
      );
    }
  }
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "Failed to decode transaction. Make sure this is an initializeChannelState transaction.",
          txHash,
          details:
...
export async function GET(request: Request) {
  try {
    ...
    console.log(
      `🔍 Searching for StateInitialized event for channel ${channelId} from block ${searchStartBlock}...`
    );
