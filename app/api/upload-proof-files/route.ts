  ...
  try {
    ...
    if (!storageBucket) {
      ...
      return NextResponse.json(
        {
          error:
            "Firebase Storage bucket not configured. Please check NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET environment variable.",
        },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error(
        "Failed to initialize storage with bucket name, trying default:",
        e
      );
