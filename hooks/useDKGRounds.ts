...
interface DKGSession {
  ...
  channelId?: string; // Optional: Link to channel in Firebase
...
export function useDKGRounds(
  ...
  const [selectedSessionForCommitment, setSelectedSessionForCommitment] =
    useState<DKGSession | null>(null);
  ...
  const submitRound1 = useCallback(
    async (
      ...
      try {
        ...
        if (!myIdHex) {
          throw new Error(
            `FROST identifier not available for session ${session.id}. Make sure ReadyRound1 message was received.`
          );
        }
        ...
        if (publicPackageHex && secretPackageHex) {
          ...
          console.log(
            "🔄 Generating new commitment packages (no pre-generated packages provided)"
          );
        ...
        const signature = signMessageECDSA(
        ...
        // Save to Firebase if channelId is available
        if (session.channelId) {
          try {
            await saveDKGMessage(session.channelId, session.id, {
              round: 1,
              messageType: "commitment",
              from: authState.dkgPublicKey,
              payload: {
                id_hex: myIdHex,
                pkg_bincode_hex: publicPkg,
              },
            });
            console.log("✅ Round 1 message saved to Firebase");
          } catch (error) {
            console.error(
              "❌ Failed to save Round 1 message to Firebase:",
              error
            );
          }
        }
        ...
        setError(
          "Failed to submit Round 1 commitment: " + (error as Error).message
        );
      } finally {
  ...
  const submitRound2 = useCallback(
    async (session: DKGSession) => {
      ...
      try {
        ...
        console.log(
          "  Round 1 packages:",
          Object.keys(sessionState.r1_packages).length
        );

        // Call real FROST DKG Part 2
        const result = dkgRound2(
          sessionState.r1_secret,
        ...
        console.log(
          "✅ DKG Part 2 complete - generated",
          Object.keys(result.outgoing_packages).length,
          "encrypted shares"
        );
        ...
        if (session.roster) {
          for (const [recipientIdHex, pkgHex] of Object.entries(
            result.outgoing_packages
          )) {
            // Find recipient's ECDSA public key from roster
            const recipient = session.roster.find(
              ([uid, idHex, ecdsaPubHex]) => idHex === recipientIdHex
            );

            if (!recipient) {
              throw new Error(
                `Could not find recipient ${recipientIdHex} in roster`
              );
            }
            ...
            console.log(
              `  Encrypting share for participant ${uid} (${idHex.slice(
                0,
                8
              )}...)`
            );
        ...
        console.log(
          "✅ Encrypted",
          encryptedPackages.length,
          "packages using ECIES"
        );
        ...
        if (session.channelId) {
          try {
            await saveDKGMessage(session.channelId, session.id, {
              round: 2,
              messageType: "secret_share",
              from: authState.dkgPublicKey,
              payload: {
                id_hex: sessionState.identifier,
                encrypted_packages_count: encryptedPackages.length,
              },
              encrypted: true,
            });
            console.log("✅ Round 2 message saved to Firebase");
          } catch (error) {
            console.error(
              "❌ Failed to save Round 2 message to Firebase:",
              error
            );
          }
        }
        ...
        setSuccessMessage(
          "🔒 Round 2 encrypted secret shares submitted! Waiting for other participants..."
        );
      } catch (error) {
        console.error("❌ Round 2 submission error:", error);
        setError(
          "Failed to submit Round 2 packages: " + (error as Error).message
        );
      } finally {
   ...
   */
  const submitFinalization = useCallback(
    async (session: DKGSession) => {
      if (!wsConnection || !authState.isAuthenticated) {
        setError("Must be connected and authenticated");
        return;
      ...
      try {
        ...
        console.log(
          "🔐 Starting DKG Finalization with real FROST cryptography..."
        );
        console.log("  Session:", session.id);
        ...
        console.log(
          "  Group public key:",
          result.group_public_key_hex.slice(0, 16) + "..."
        );
        ...
        if (session.channelId) {
          try {
            ...
            console.log("✅ Finalization message saved to Firebase");
          } catch (error) {
            console.error(
              "❌ Failed to save finalization message to Firebase:",
              error
            );
          }
        }
        ...
        setSuccessMessage(
          "🎯 Finalization submitted! Computing final group verification key..."
        );
        ...
        try {
          ...
          localStorage.setItem(
            `dkg_key_package_${session.id}`,
            JSON.stringify(keyPackageData)
          );
          console.log("💾 Key package saved to localStorage");
        } catch (storageError) {
          console.warn(
            "⚠️ Could not save key package to localStorage:",
            storageError
          );
        }
      } catch (error) {
   ...
   */
  const handleRound1All = useCallback(
    (sessionId: string, packages: Array<[string, string, string]>) => {
      ...
      console.log(
        "✅ Stored",
        Object.keys(packagesMap).length,
        "Round 1 packages (excluding self)"
      );
    },
    []
  );
   ...
   */
  const handleRound2All = useCallback(
    async (
      sessionId: string,
      packages: Array<[string, string, string, string, string]>
    ) => {
      console.log(
        "📦 Received encrypted Round 2 packages for session",
        sessionId
      );
      console.log("  Total packages:", packages.length);
      ...
      try {
        ...
        console.log(
          ...
          Object.keys(decryptedPackages).length,
          "Round 2 packages"
        );
      } catch (error) {
        console.error("❌ Failed to decrypt Round 2 packages:", error);
        setError(
          "Failed to decrypt Round 2 packages: " + (error as Error).message
        );
      }
    },
