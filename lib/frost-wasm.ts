...
/**
 * Decompress a SEC1 compressed public key (33 bytes) into X and Y coordinates
 * Works for secp256k1 curve: y² = x³ + 7 (mod p)
 * 
 * @param compressedHex - Compressed SEC1 public key (66 hex chars, starts with 02 or 03)
 * @returns Object with px and py as 0x-prefixed hex strings, or null if invalid
 */
export function decompressPublicKey(compressedHex: string): { px: string; py: string } | null {
  try {
    const cleanHex = compressedHex.replace(/^0x/, '').toLowerCase();
    
    // Must be 66 hex chars (33 bytes)
    if (cleanHex.length !== 66) {
      console.warn('Invalid compressed key length:', cleanHex.length);
      return null;
    }
    
    const prefix = cleanHex.substring(0, 2);
    const xHex = cleanHex.substring(2);
    
    // Prefix must be 02 (even y) or 03 (odd y)
    if (prefix !== '02' && prefix !== '03') {
      console.warn('Invalid prefix for compressed key:', prefix);
      return null;
    }
    ...
    // Compute modular square root using Tonelli-Shanks (p ≡ 3 mod 4 for secp256k1)
    // y = y2^((p+1)/4) mod p
    const y = modPow(y2, (p + 1n) / 4n, p);
    
    // Verify the square root
    if ((y * y) % p !== y2) {
      console.warn('Invalid point: not on curve');
      return null;
    }
    
    // Choose correct y based on prefix (02 = even, 03 = odd)
    const isOdd = y % 2n === 1n;
    const expectedOdd = prefix === '03';
    const finalY = isOdd === expectedOdd ? y : p - y;
    
    // Convert to hex strings (32 bytes each, 0x-prefixed)
    const px = '0x' + x.toString(16).padStart(64, '0');
    const py = '0x' + finalY.toString(16).padStart(64, '0');
